    ...
    def atualizar_treeview(self):
        ...
        for i, nota in enumerate(self.dados_atuais):
            ...
            self.tree.insert("", tk.END, 
                           text=str(i + 1),
                           values=(
                               nota.get('chave_acesso', '')[:20] + "..." if len(nota.get('chave_acesso', '')) > 20 else nota.get('chave_acesso', ''),
                               nota.get('numero', ''),
                               nota.get('data_emissao', ''),
                               (nota.get('nome_emissor', '') or nota.get('cnpj_emissor', ''))[:30] + "..." if len((nota.get('nome_emissor', '') or nota.get('cnpj_emissor', ''))) > 30 else (nota.get('nome_emissor', '') or nota.get('cnpj_emissor', '')),
                               (nota.get('nome_destinatario', '') or nota.get('cnpj_destinatario', ''))[:30] + "..." if len((nota.get('nome_destinatario', '') or nota.get('cnpj_destinatario', ''))) > 30 else (nota.get('nome_destinatario', '') or nota.get('cnpj_destinatario', '')),
...
"""
    ...
    def processar_xmls_thread(self, pasta_xml):
        """Processa XMLs em thread separada"""
        
        def processar():
            try:
                # Aqui você integraria com seu NFeProcessor existente
                # Por agora, simulamos o processamento
                messagebox.showinfo("Processamento", "Iniciando processamento de XMLs...")
                
                # Atualiza dados após processamento
                self.after(1000, self.aplicar_filtros)
                
            except Exception as e:
                messagebox.showerror("Erro", f"Erro ao processar XMLs: {e}")
    ...
    def filtrar_por_empresa(self):
        ...
        if selection:
            ...
            # Aqui você implementaria o filtro por empresa
            messagebox.showinfo("Info", f"Filtrar por empresa CNPJ: {cnpj}")
    ...
    def copiar_chave(self):
        ...
        selection = self.tree.selection()
        if selection:
            chave = self.tree.item(selection[0])['values'][0]
            self.clipboard_clear()
            self.clipboard_append(chave)
            messagebox.showinfo("Copiado", "Chave copiada para clipboard")
    ...
    def executar_analise_ia(self):
        ...
        messagebox.showinfo("Info", "Análise de IA em desenvolvimento")
    
    def excluir_nota(self):
        """Exclui nota selecionada"""
        if messagebox.askyesno("Confirmar", "Deseja excluir esta nota fiscal?"):
